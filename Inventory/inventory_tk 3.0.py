...
class InventoryApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("V.A.V.H Studio")
        self.geometry("950x600")
        self.resizable(True, True)
        self.configure(bg="#0b0c0d")
        self.style = ttk.Style(self)
        self.style.theme_use("clam")
        self.style.configure("TFrame", background="#0b0c0d")
        self.style.configure("TLabel", background="#0b0c0d", foreground="white")
        self.style.configure("TButton", background="#00ff99", foreground="black", font=("Arial", 10, "bold"))
        self.style.map("TButton", background=[("active", "#33ffaa")])
    ...
    def create_login_screen(self):
        ...
        ttk.Label(frame, text="V.A.V.H Studio", font=("Arial", 22, "bold"), foreground="#00ff99").grid(row=0, column=0, columnspan=2, pady=(0, 20))
    ...
    def create_main_ui(self):
        ...
        ttk.Label(top, text=f"Logged in: {self.current_user['username']} ({self.current_user['role']})", foreground="#00ff99").pack(side="left")
        ...
        listbox = tk.Listbox(left, bg="#1a1a1a", fg="#00ff99", selectbackground="#00ff99", selectforeground="black")
        ...
        details = tk.Text(right, height=15, bg="#111", fg="#00ff99", insertbackground="#00ff99")
        ...
        def create_user():
            uname = simpledialog.askstring("New User", "Enter new username:", parent=self)
            if not uname:
                return
        ...
        if role == "admin":
            ...
            ttk.Button(action_frame, text="Create User", command=create_user).pack(side="left", padx=4)
        else:
            ttk.Button(action_frame, text="Adjust Stock", command=adjust_stock).pack(side="left", padx=4)
            ttk.Button(action_frame, text="Low Stock Report", command=low_stock_report).pack(side="left", padx=4)
            ttk.Button(action_frame, text="View History", command=view_history).pack(side="left", padx=4)
